...
namespace RTS.Detectors
{
    public class ResourcesDetector : MonoBehaviour
    {
        ...
        private bool TryGetResources(out List<Resource> resources)
        {
            ...
            {
                if (resourceCollider.TryGetComponent(out Resource resource) && resource.IsTaken == false)
                {
                    resource.Taken += OnResorceTake;
        ...
        private void OnResorceTake(Resource resource)
        {
            resource.Taken -= OnResorceTake;
            _resourcesQueue.Remove(resource);
        }
    }
}